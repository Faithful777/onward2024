...
class SegmentationModel(nn.Module):
    ...
    def __init__(self, backbone, neck, decode_head):
        ...
        #self.checkpoint = torch.load(self.starting_weights,
        #                          map_location=torch.device('cpu'))
        #print(self.checkpoint)
        #self.backbone.load_state_dict(self.checkpoint, strict=False)
        self.neck = neck
...
def finetune_vit(dataset: str,
    ...
    for name, param in checkpoint.items():
        print(f'this is the {name} from backbone')
    ...
    dataset = torchvision.datasets.ImageFolder(root=dataset, img_size=224, transform=transform, target_transform=transform)
